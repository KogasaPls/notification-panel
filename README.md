...
A rule is imported **enabled only when the wildcard matches exactly the same messages the regular expression did**.
Plain patterns qualify, because the old matching already searched within the message, so `dragon` and `.*dragon.*`
both become an exact equivalent. Anything that would widen the match arrives turned off, because a silently widened
rule is the worst outcome: with "Show notifications by default" off it would quietly admit notifications you had
suppressed, and nothing would point at the upgrade as the cause.

Two translations widen a pattern. Both keep their **converted** text, so reviewing one and switching it on is all
that is needed:

* **Anchors are dropped.** `^Congratulations$` becomes `Congratulations`, which matches any message containing that
  word rather than only a message that is exactly that word. Wildcards have no anchor to translate to.
* **A lone `.` becomes `*`.** `.` matched exactly one character; `*` matches any run of them, so `level .` becomes
  `level *`. Wildcards have no single-character metacharacter — `?` is a literal.

Patterns that cannot be expressed as wildcards at all — alternation, groups, character classes — arrive turned off
too, but keep their **original** text, since you have to rewrite them by hand. A pattern built only from lone dots
(`.`, `..`, `...`) is treated this way as well: it would collapse to a bare `*` matching every notification, which is
almost certainly not what it meant. A genuine catch-all like `.*` is imported enabled, because it already matched
everything.

Look over your imported rules after upgrading; anything turned off is flagged in the sidebar with what it needs.
